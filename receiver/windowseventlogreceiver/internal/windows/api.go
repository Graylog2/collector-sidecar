...
const (
	// Stale bookmark error codes.
	ErrorNotFound                      syscall.Errno = 1168
	ErrorEvtQueryResultStale           syscall.Errno = 15011
	ErrorEvtQueryResultInvalidPosition syscall.Errno = 15012
)
...
// Event metadata property IDs for EvtGetEventMetadataProperty.
// https://learn.microsoft.com/en-us/windows/win32/api/winevt/ne-winevt-evt_event_metadata_property_id
const (
	EvtEventMetadataEventID        uint32 = 0
	EvtEventMetadataEventVersion   uint32 = 1
	EvtEventMetadataEventMessageID uint32 = 6
)

// evtOpenEventMetadataEnum is the direct syscall implementation of EvtOpenEventMetadataEnum (https://learn.microsoft.com/en-us/windows/win32/api/winevt/nf-winevt-evtopeneventmetadataenum)
var evtOpenEventMetadataEnum = func(publisherMetadata uintptr, flags uint32) (uintptr, error) {
	handle, _, err := openEventMetadataEnumProc.Call(publisherMetadata, uintptr(flags))
	if !errors.Is(err, ErrorSuccess) {
		return 0, err
	}
...
// evtNextEventMetadata is the direct syscall implementation of EvtNextEventMetadata (https://learn.microsoft.com/en-us/windows/win32/api/winevt/nf-winevt-evtnexteventmetadata)
var evtNextEventMetadata = func(eventMetadataEnum uintptr, flags uint32) (uintptr, error) {
	handle, _, err := nextEventMetadataProc.Call(eventMetadataEnum, uintptr(flags))
	if !errors.Is(err, ErrorSuccess) {
		return 0, err
	}
...
// evtGetEventMetadataProperty is the direct syscall implementation of EvtGetEventMetadataProperty (https://learn.microsoft.com/en-us/windows/win32/api/winevt/nf-winevt-evtgeteventmetadataproperty)
var evtGetEventMetadataProperty = func(eventMetadata uintptr, propertyID, flags, bufferSize uint32, buffer *byte) (*uint32, error) {
	bufferUsed := new(uint32)
	_, _, err := getEventMetadataPropertyProc.Call(eventMetadata, uintptr(propertyID), uintptr(flags), uintptr(bufferSize), uintptr(unsafe.Pointer(buffer)), uintptr(unsafe.Pointer(bufferUsed)))
	if !errors.Is(err, ErrorSuccess) {
		return bufferUsed, err
	}
