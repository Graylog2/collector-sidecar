...
func (b *Buffer) ReadWideChars(offset uint32) ([]byte, error) {
	byteOffset := uint64(offset) * bytesPerWChar
	if byteOffset > uint64(^uint32(0)) {
		return nil, fmt.Errorf("wide char offset %d overflows uint32", offset)
	}
	return b.ReadBytes(uint32(byteOffset))
}
